...
from metrics.performance_metrics import (compute_auc_score,
                                             compute_dauprc_score)
from metrics.tracker import (MovingAverageTracker,
                                 TrainValDeltaTracker)
from mhnfs.optimizer import define_opimizer
...
class MHNfs(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # needed to add this to load correct config files
        with initialize(config_path="../mhnfs/configs/", job_name=None):
            config = compose(config_name="cfg")
        ...
        self.context = (
            torch.unsqueeze(
                torch.from_numpy(
                    np.load(
                        self.cfg.system.data.path
                        + self.cfg.system.data.dir_training
                        + self.cfg.system.data.name_mol_inputs
                    )
                ),
                0,
            )
            .float()
            .to(self.cfg.system.ressources.device) # .to('cpu') #
        )
        ...
        self.layerNorm_context = torch.nn.LayerNorm(
            ...
            elementwise_affine=self.cfg.model.layerNormBlock.affine,
        )
